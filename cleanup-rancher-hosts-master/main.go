...
func httpGetRequest(path string) (res *http.Response, err error) {
...
func httpDeleteRequest(path string) (err error) {
	client := createHttpClient()
...
func httpPostRequest(path string) (err error) {
	client := createHttpClient()
...
func main() {
	flag.StringVar(baseURLPtr, "u", "", "Base URL do Rancher")
	flag.StringVar(accessKeyPtr, "a", "", "Access Key do Rancher")
	flag.StringVar(secretKeyPtr, "s", "", "Secret Key do Rancher")
	flag.StringVar(projectIDPtr, "p", "", "Project ID do Rancher")
	flag.Parse()
	...
	resp, err := httpGetRequest(fmt.Sprintf("/projects/%s/hosts", projectID))
	if err != nil {
		panic(err)
	}
	...
	for _, host := range hostResp.Data {
		if host.AgentState == "disconnected" {
			err := httpPostRequest(fmt.Sprintf("/projects/%s/hosts/%s", projectID, host.ID))
			if err != nil {
				log.Printf("[ERROR] Erro ao inativar host de ID %s\n", host.ID)
			} else {
				err = httpDeleteRequest(fmt.Sprintf("/projects/%s/hosts/%s", projectID, host.ID))
				if err != nil {
					log.Printf("[ERROR] Erro ao deletar host de ID %s\n", host.ID)
				} else {
					log.Printf("[INFO] Host de ID %s removido com sucesso!\n", host.ID)
				}
			}
		}
	}
